...
class LawyerBloc extends Bloc<LawyerEvent, LawyerState> {
  ...
  Future<void> _mapFetchLawyersToState(
    ...
    try {
      ...
      final nextPageResult = await apiProvider.fetchLawyersBySpecialization(
          page: event.page,
          limit: event.limit,
  ...
  try {
    // Decide which API method to call based on specializationId
    if (event.specializationId != null) {
      // Search lawyers in a specific specialization
      ...
      // Search lawyers in general (no specialization)
      final searchResult = await apiProvider.searchLawyersByName(
        event.query,
        page: event.page,
        limit: event.limit,
      );
